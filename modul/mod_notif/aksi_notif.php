<?php
	...
	elseif($_GET['module']=='update'){
			$query=$db->query("UPDATE ms_employee_dep SET depname='$depname',editby='$userid',editdt='$sdate' WHERE  concat(eid,seqno)='$id'");
			header("location:../../media.php?module=doc&&id=".$regid);
	}
	elseif($_GET['module']=='delete'){
		$id=$_GET['sedit'];
		$regid=substr($id,0,12);
		$query=$db->query("DELETE FROM tr_document WHERE regid='$id'");
		header("location:../../media.php?module=doc&&id=".$regid);
	}
?>
