<?php
	...
	$aColumns = array( 'regid','produk','nama','tgllahir','cabang','mulai','up','premi','status','aksi' );
	...
	$sTable = "(SELECT aa.regid, 
                       ...
                       aa.premi, 
                       ab.msdesc 'status', 
                       concat(aa.regid,'-',aa.status) 'aksi' 
                       
                FROM   (
						select * from tr_sppa where status in ('5','6','20' )  ";
	...
	if ($_POST['sSearch'] !== '') {
		...
		$sTable .= " 					) aa
                       ...
                       LEFT JOIN (SELECT msid, msdesc FROM ms_master WHERE mstype = 'CAB') ac 
                              ON ac.msid = aa.cabang
                       INNER JOIN (SELECT msid,msdesc FROM ms_master WHERE mstype='PRODUK') ad
                              ON ad.msid = aa.produk
