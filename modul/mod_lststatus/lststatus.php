                                ...
                                <div class="x_content">
                                    ...
                                    <table class="table table-bordered">
										...
										<?php
											...
											$sqld="select c.msdesc cab, t.createby ao,a.regid,t.nama,m.msdesc 'status', ";
											$sqld= $sqld . "a.createdt tglpros,a.createby 'user',concat(t.`comment`,';',a.comment) remak from  ";
											$sqld= $sqld . " ( select regid,createdt,createby,comment,status,datediff(now(),createdt) sdur ";
											$sqld= $sqld . " from tr_sppa_log where datediff(now(),createdt)<=10 ) a ";
											$sqld= $sqld . "inner join ms_master m on a.status=m.msid and m.mstype='streq' ";
											$sqld= $sqld . "inner join tr_sppa t on t.regid=a.regid ";
											$sqld= $sqld . "inner join ms_master c on t.cabang=c.msid and c.mstype='cab' "; 
											
											
											if ($vlevel=="checker" or $vlevel=="schecker")
																	
											{
											$sqld= $sqld . "where sdur<=7 ";
											$sqld= $sqld . "and t.cabang ='$ucab'";
											}
											
											if ($vlevel=="mkt"  )
											{
											$sqld= $sqld . "where datediff(now(),a.createdt)<=10 and t.createby ='$userid'";
											}
																						
											if ($vlevel=="smkt" )
											{
											$sqld= $sqld . "where sdur<=10 and t.createby in ";
											$sqld= $sqld . " (select distinct case when a.parent=a.username ";
											$sqld= $sqld . " then a.parent else a.username end from ms_admin a ";
											$sqld= $sqld . " where (a.username='$userid' or a.parent='$userid')) ";
											}
											if ($vlevel=="insurance" or $vlevel=="broker" )
											{
											$sqld= $sqld . "where sdur<=3 ";
											}
											$sqld= $sqld . " order by a.createdt  desc  LIMIT $posisi,$batas" ;
										...
										?>
                                            <tr>
												...
												<td><?php echo $r['tglpros']; ?></td>
												<td><?php echo $r['user']; ?></td>
												<td><?php echo $r['remak']; ?></td>
									...
									<?php
							$sqlt=" select a.regid from (select regid,createdt,datediff(now(),createdt) sdur from tr_sppa_log where datediff(now(),createdt)<=7 ) a ";
							$sqlt= $sqlt . "inner join tr_sppa t on a.regid=t.regid ";
							if ($vlevel=="checker" or $vlevel=="schecker")
							{
							$sqlt= $sqlt . " where sdur<=7 and t.cabang='$ucab'";
							}

							if ($vlevel=="insurance" or $vlevel=="broker" )
							{
							$sqlt= $sqlt . " where sdur<=3 ";
							}
							if ($vlevel=="mkt"  )
							{
							$sqlt= $sqlt . " where sdur<=10 and t.createby='$userid' ";
							}
							if ($vlevel=="smkt" )
							{
							$sqlt= $sqlt . "where sdur<=10 and t.createby in ";
							$sqlt= $sqlt . " (select distinct case when a.parent=a.username ";
							$sqlt= $sqlt . " then a.parent else a.username end from ms_admin a ";
							$sqlt= $sqlt . " where (a.username='$userid' or a.parent='$userid')) ";
							}
							/* echo $sqlt; */
