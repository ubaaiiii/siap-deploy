                ...
                <script>
                    $(document).ready(function(){
                			  ...
                			  {                         // full[11] => Aksi
                				"mRender": function ( data, type, full ) {
                				    ...
                				    if (('<?=$vlevel;?>' == 'broker' && (status == '90' || status == '91')) || ('<?=$vlevel;?>' == 'checker' && status == '90')) {
                				        var button = `<a class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="top" title="Ubah Data" href="media.php?module=claim&&act=update&&id=`+regid+`"><i class="fa fa-edit"></i> Edit</a>`;
                				        button += `<a class="btn-batal btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Batal Claim" data-id="`+regid+`"><i class="fa fa-times"></i> Batal</a>`;
                				    } else {
                				        var button = `<a class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="top" title="Lihat Data" href="media.php?module=claim&&act=view&&id=`+regid+`"><i class="fa fa-search"></i> View</a>`;
                				    }
                				    
                				    button += `<a class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="top" title="View Documents" href="media.php?module=doc&&id=`+regid+`&&jenis=DTCLM"><i class="fa fa-files-o"></i> Doc</a>`;
                				    button += `<a class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="top" title="Download Checklist Document" href="laporan/claim/f_claim.php?id=`+regid+`"><i class="fa fa-download"></i> List</a>`;
                				    
                				    
                				    if ('<?=$vlevel;?>' == 'broker' && (status == '91' || status == '90')) {
                				        button += `<a class="btn-approve btn btn-xs btn-info" data-toggle="tooltip" data-placement="top" title="Approve" data-regid="`+regid+`" data-id="`+full[0]+`"><i class="fa fa-check"></i> `+kelengkapan+`</a>`;
                				        if (status == '91') {
                    				        button += `<button type="button" class="btn-rollback btn btn-xs btn-warning" data-toggle="tooltip" data-placement="top" title="Rollback" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-undo"></i> Rollback</button>`;  
                				        }
                				    }
                				    
                				    if ('<?=$vlevel;?>' == 'schecker' && status == '90') {
                				        button += `<a class="btn-approve btn btn-xs btn-info" data-toggle="tooltip" data-placement="top" title="Approve" data-regid="`+regid+`" data-id="`+full[0]+`"><i class="fa fa-check"></i> Approve</a>`;
                				    }
                				    
                				    if ('<?=$vlevel;?>' == 'insurance') {
                				        if (status == '90') {
                				            button += `<a class="btn-reject btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Reject" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-warning"></i> Expired</a>`;
                				        } else if (status == '92') {
                				            button += `<a class="btn-approve-ins btn btn-xs btn-info" data-toggle="tooltip" data-placement="top" title="Approve" data-regid="`+regid+`" data-id="`+full[0]+`"><i class="fa fa-check"></i> Approve</a>`;
                    				        button += `<a class="btn-reject btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Reject" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-times"></i> Reject</a>`;
                    				        button += `<button type="button" class="btn-rollback btn btn-xs btn-warning" data-toggle="tooltip" data-placement="top" title="Rollback" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-undo"></i> Rollback</button>`;  
                				        } else if (status == '93') {
                				            button += `<a class="btn-receive btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" title="Dokumen Telah Diterima" data-id="`+regid+`"><i class="fa fa-check-circle"></i> Received</a>`;
                				            button += `<a class="btn-reject btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Reject" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-times"></i> Reject</a>`;
                    				        button += `<button type="button" class="btn-rollback btn btn-xs btn-warning" data-toggle="tooltip" data-placement="top" title="Rollback" data-id="`+regid+`" data-nama="`+nama+`"><i class="fa fa-undo"></i> Rollback</button>`;  
                				        } else if (status == '96') {
                				            button += `<a class="btn-paid btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" data-nil="`+full[8]+`" data-id="`+regid+`" title="Pembayaran Claim"><i class="fa fa-calendar"></i> Paid</a>`;
                				        }
                				    }
                				    
                				    button += `<a class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="top" title="History Log" href="media.php?module=polhist&&act=log&&type=LTCLM&&id=`+regid+`"><i class="fa fa-history"></i> Log</button>`;
                				    
                				    return button;
                				}
                			  },
                			  {"sName": "reg_encode", "visible": false},     // full[12] => regid yang udah diencrypt
                			],
                			"columnDefs": [
                			    {
                        ...
                        $('#table-claim tbody').on('click', 'a.btn-receive', function() {
                            ...
                            swal.fire({
                                title: 'Masukkan tanggal bahwa dokumen hardcopy telah diterima dan telah disetujui kebenarannya',
                                html: "<input type='date' class='form-control' required style='width:100%;text-align: center;' id='tglreceive' name='tglreceive'><br>",
                                icon: 'info',
                                showCancelButton: true,
                            }).then(function(result) {
                                if (result.dismiss != 'cancel' && result.dismiss != 'backdrop' && result.dismiss != 'esc') {
                                    if ($('#tglreceive').val()) {
                                        $.ajax({
                                            url: "<?=$aksi;?>?module=receive",
                                            data: {"regid":id,"tgl":$('#tglreceive').val()},
                                            type: "post",
                                            success: function(data) {
                                                if (data == 'berhasil') {
                                        ...
                                        Swal.fire(
                                            'Terima Dokumen Gagal!',
                                            'Harap mengisi tanggal terima dokumen',
                                            'error'
                                        );
                                    }
                                }
                        ...
                        <?php
                            $query = $db->query("SELECT msid,msdesc FROM ms_master WHERE mstype='REJECTTYPE'");
...
case "add":
    ...
    $queryReg = $db->query("SELECT Concat(Concat(prevno, Date_format(Now(), '%y%m')), RIGHT( 
                                           Concat(formseqno, b.lastno), formseqlen)) AS seqno 
                             FROM   tbl_lastno_form a 
                                    LEFT JOIN tbl_lastno_trans b 
                                           ON a.trxid = b.trxid 
                             WHERE  a.trxid = 'regclm' ");
    $d        = $queryReg->fetch_array();
    $regclaim = $d['seqno'];
    
    $updReg = $db->query("UPDATE tbl_lastno_trans 
                           SET    lastno = lastno + 1 
                           WHERE  trxid = 'regclm' ");
                    ...
                    <div class="x_content">
                        ...
                        <form method="post" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" action="<?= $aksi."?module=".$action; ?>">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="text" id="regid" readonly required class="form-control col-md-7 col-xs-12" value="<?= $r['regid']; ?>">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    ...
                                    <input type="date" value="<?= $r['tgllapor']; ?>" id="tgllapor" name="tgllapor" value="yyyy-mm-dd" required class="range-tgl form-control col-md-7 col-xs-12">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="date" value="<?= $r['tglkejadian']; ?>" id="tglkejadian" name="tglkejadian" value="yyyy-mm-dd" required class="range-tgl form-control col-md-7 col-xs-12">
                            ...
                            <div class="form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<textarea name="subject" rows="5" class="textbox" id="subject" style="width: 98%;" data-parsley-id="8971"><?=$r['comment'];?></textarea><ul class="parsley-errors-list" id="parsley-id-8971"></ul>
								</div>
							</div>
                            <?php
                                if ($action == 'update') {
