<?php
    function detectTgl($date) {
        if (strpos($date, '/') !== false) {
    	    $tgl = explode("/", $date);
    	    $date = $tgl[2]."-".$tgl[1]."-".$tgl[0];
    	} else {
    	    $tgl = explode("-", $date);
    	    if (strlen($tgl[0]) !== 4) { // format selain yyyy-mm-dd
    	        $date = $tgl[2]."-".$tgl[1]."-".$tgl[0];
    	    }
    	}
    	return $date;
...
// 	$sregid        = encrypt_decrypt("decrypt",$_POST['regid']);
	$sregid        = $_POST['regid'];
	$stglkejadian  = detectTgl($_POST['tglkejadian']);
	$stgllapor     = detectTgl($_POST['tgllapor']);
    ...
    if (isset($_SESSION['idLog'])) {
        if($_GET['module']=='add'){
            if ($akhir < $stglkejadian) {
                echo "<script>
                        alert('Tidak dapat dicover asuransi, dikarenakan masa periode sudah selesai.');
                        window.location = '../../media.php?module=claim';
                      </script>";
            }
